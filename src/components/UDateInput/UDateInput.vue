<script setup lang="ts">
  /**
   * Extended UDateInput component built on Vuetify's VDateInput.
   * Inherits all VDateInput props, slots, and emits.
   *
   * Supports two modes:
   * 1. Default mode (no variant): Custom styled with external label, outlined variant, and compact density
   * 2. Standard mode (with variant): Full passthrough to VDateInput
   *
   * @component
   * @extends VDateInput
   */
  ...
  // Component options
  defineOptions({ name: 'UDateInput', inheritAttrs: false });
  ...
  const containerStyle = computed(() => {
    ...
    console.log('Attrs for containerStyle:', attrs);
  ...
  // Filter out width-related props from attrs for VDateInput
...
<template>
  ...
  <v-date-input v-if="variant" v-bind="$attrs">
  ...
  <div v-else class="v-input" :style="containerStyle">
    ...
    <v-date-input v-bind="inputAttrs" :id="elementId" label="" variant="outlined" density="compact">
      <template v-for="(_, name) in $slots" :key="name" #[name]="slotData">
        <slot :name="name" v-bind="slotData || {}" />
      </template>
    </v-date-input>
