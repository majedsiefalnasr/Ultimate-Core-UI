<script setup lang="ts">
  /**
   * Extended UMaskInput component built on Vuetify's VMaskInput.
   * Inherits all VMaskInput props, slots, and emits.
   *
   * Supports two modes:
   * 1. Default mode (no variant): Custom styled with external label, outlined variant, and compact density
   * 2. Standard mode (with variant): Full passthrough to VMaskInput
   *
   * @component
   * @extends VMaskInput
   */
  ...
  import './UMaskInput.scss';

  // Component options
  defineOptions({ name: 'UMaskInput', inheritAttrs: false });
  ...
  // Filter out width-related props from attrs for VMaskInput
...
<template>
  ...
  <v-mask-input v-if="variant" v-bind="$attrs">
