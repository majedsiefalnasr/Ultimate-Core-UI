...
// Open And Close Delay Story
const openAndCloseDelayTemplate = `
  <u-row>
    <u-col cols="12" sm="6">
 ...
 */
export const OpenAndCloseDelay: StoryFn<ComponentArgs> = () => ({
  components: { UHover, UCard, UCardText, URow, UCol },
  template: openAndCloseDelayTemplate,
});

OpenAndCloseDelay.parameters = {
  docs: {
    source: {
      code: `<template>${openAndCloseDelayTemplate}</template>
...
/**
 * u-hover can be used in combination with v-for to make a single item stand out when the
 * user interacts with the list.
 */
export const HoverList: StoryFn<ComponentArgs> = () => ({
  ...
  setup() {
    ...
    return { icons, items, transparent };
  },
  template: hoverListTemplate,
});

HoverList.parameters = {
  docs: {
    source: {
      code: `<template>${hoverListTemplate}</template>
 ...
 */
export const Transition: StoryFn<ComponentArgs> = () => ({
...
Transition.parameters = {
  docs: {
    source: {
      code: `<template>${transitionTemplate}</template>
