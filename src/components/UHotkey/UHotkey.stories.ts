...
/**
 * The useHotkey composable provides keyboard shortcut functionality.
 */
export const UseHotkey: StoryFn<ComponentArgs> = () => ({
  ...
  template: useHotkeyTemplate,
});

UseHotkey.parameters = {
  docs: {
    source: {
      code: `<template>${useHotkeyTemplate}</template>
...
/**
 * Create multi-step keyboard shortcuts by separating keys with dashes. Users must press keys
 * in sequence within the timeout period.
 */
export const KeySequences: StoryFn<ComponentArgs> = () => ({
  components: {
    UContainer,
    URow,
    UCol,
  ...
  setup() {
    ...
    return { messages, sequenceTimeout, clearMessages, updateTimeout };
  },
  template: keySequencesTemplate,
});

KeySequences.parameters = {
  docs: {
    source: {
      code: `<template>${keySequencesTemplate}</template>
...
// Platform Awareness Story
const platformAwarenessTemplate = `
  <u-container>
    <u-row>
      <u-col cols="12">
        <u-card>
...
/**
 * The composable automatically handles platform differences. Use cmd for cross-platform
 * compatibility or specific modifiers for platform-specific behavior.
 */
export const PlatformAwareness: StoryFn<ComponentArgs> = () => ({
  components: {
    UContainer,
    URow,
    UCol,
  ...
  setup() {
    ...
    return { messages, isMac, platformName, userAgent, clearMessages };
  },
  template: platformAwarenessTemplate,
});

PlatformAwareness.parameters = {
  docs: {
    source: {
      code: `<template>${platformAwarenessTemplate}</template>
...
// Reactive Hotkeys Story
const reactiveHotkeysTemplate = `
  <u-container>
    <u-row>
      <u-col cols="12">
        <u-card>
 ...
 */
export const ReactiveHotkeys: StoryFn<ComponentArgs> = () => ({
  components: {
    UContainer,
    URow,
    UCol,
  ...
  template: reactiveHotkeysTemplate,
});

ReactiveHotkeys.parameters = {
  docs: {
    source: {
      code: `<template>${reactiveHotkeysTemplate}</template>
...
<script setup>
  ...
  watch([saveHotkey, hotkeyEnabled], () => {
    addMessage('⚙️ Hotkey configuration updated')
  })
</script>`,
    },
 ...
 */
export const Options: StoryFn<ComponentArgs> = () => ({
  components: {
    UContainer,
    URow,
    UCol,
...
Options.parameters = {
  docs: {
    source: {
      code: `<template>${optionsTemplate}</template>
