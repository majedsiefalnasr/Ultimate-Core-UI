    ...
    docs: {
      ...
      source: {
        transform: (src: string, storyContext: { args: ComponentArgs }) => {
          ...
          return `
  <u-layout${attrsString} class="rounded rounded-md">
...
// ComplexLayouts Story
const complexLayoutsTemplate = `
  <u-layout class="rounded rounded-md">
    <u-system-bar color="grey-darken-3"></u-system-bar>
...
/**
 * Complex layout with multiple navigation drawers and app bars.
 */
export const ComplexLayouts: StoryFn<ComponentArgs> = () => ({
  components: {
    ULayout,
    USystemBar,
    UNavigationDrawer,
  ...
  template: complexLayoutsTemplate,
});

ComplexLayouts.parameters = {
  docs: {
    source: {
      code: `<template>${complexLayoutsTemplate}</template>`,
    },
  },
};
...
/**
 * Dynamic layout with controllable app bar order.
 */
export const DynamicOrder: StoryFn<ComponentArgs> = () => ({
  components: { ULayout, UNavigationDrawer, UAppBar, UMain, UContainer, USheet, USwitch },
  setup() {
    const order = shallowRef(0);
    return { order };
  },
  template: dynamicOrderTemplate,
});

DynamicOrder.parameters = {
  docs: {
    source: {
      code: `<template>${dynamicOrderTemplate}</template>
